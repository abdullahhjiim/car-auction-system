...
import CommonCarList from '@/components/admin/CommonCarList';
...
const OnApproval = () => {
  ...
  const fetchData = (urlStr) => {
    setLoading(true);
    const url = urlStr
      ? `/member-vehicles/pending${urlStr}`
      : '/member-vehicles/pending';
    authAxios
      .get(url, {
            ...
            <div className="w-full lg:w-2/3 xl:w-3/4">
              <BreadCrumbs
                breadCumbsArr={[
                  ...
                  { link: '', text: 'On Approval Cars' },
                ]}
              />
              <CommonCarList
                name="On Approval Cars"
                tableBodyData={data}
                token={token}
                handleSearch={handleSearch}
                dataLoading={loading}
                url={'/admin/sell-my-car/on-approval'}
