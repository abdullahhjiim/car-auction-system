...
import { FaMailBulk, FaThList, FaTimes, FaWhatsapp } from "react-icons/fa";
...
const AuctionVehicles = () => {
  ...
  const _fetchLotData = (params, firstLoading) => {
    let apiUrl = "/lot/in-auction-vehicles";
    if (!firstLoading) {
      if (auction_id) {
        apiUrl = apiUrl + `/${auction_id}`;
      }
    }
  ...
  const _handleOrderByPrice = (e) => {
    ...
    if (value.length > 0) {
      const data = {
        ...filterData,
        order_by_column: "start_bid_amount",
        order_by: value,
      };
  ...
  const handleExport = () => {
    if (token) {
      ...
      authAxios
        .get(`/auctions/${auction_id}/car-listing`, {
          responseType: "blob",
          headers: {
        ...
        <div className="col-span-3 lg:col-span-1">
          ...
          >
              ...
              <div>
                ...
                >
                  {filterData.newly_added_vehicle &&
                    filterData.newly_added_vehicle.map((e, i) => {
                      return (
            ...
            <div className="flex justify-between items-center mt-4">
              <div className="flex md:flex-row justify-center gap-2">
                <div className="flex justify-center items-center gap-1 bg-[#128C7E] p-2 item-ceter rounded-md text-white">
                  <FaWhatsapp />
                  <Link
                    className=""
                    target="_blank"
                    href={`https://web.whatsapp.com/send?text=Checkout this car from ${process.env.NEXT_PUBLIC_APP_NAME}: https://gulfauctions.online/auction-vehicle?auction_id=${auction_id}`}
                  >
                    {`WhatsApp`}
                  </Link>
                </div>
                <div className="flex justify-center items-center gap-x-1 bg-primary py-1 px-3 item-ceter rounded-md text-white">
                  <FaMailBulk />
                  <Link href={`mailto:info@arianashippingline.com`}>
                    {`Mail`}
                  </Link>
                </div>
                {auction_id && (
                  <div
                    ...
                    className="flex bg-primary p-2 items-center gap-1 rounded-md text-white cursor-pointer"
                  >
                    <MdFileDownload />
                    {printLoading && (
                      <svg
                        ariaHidden="true"
                        className="w-4 h-4 text-gray-200 animate-spin dark:text-gray-600 fill-blue-600 mt-2"
                        viewBox="0 0 100 101"
                        fill="none"
