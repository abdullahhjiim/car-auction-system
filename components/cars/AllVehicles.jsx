...
const AllVehicles = () => {
  ...
  const priceRange = [0, 200000];

  if (queryObject?.start_bid_amount) {
    priceRange[0] = parseInt(queryObject?.start_bid_amount.split("-")[0]);
    priceRange[1] = parseInt(queryObject?.start_bid_amount.split("-")[1]);
  }
  ...
  const _handleSubmitPriceRange = (e) => {
    e.preventDefault();

    if (values.length > 0) {
      const data = {
        ...filterData,
        start_bid_amount: `${values[0]}-${values[1]}`,
      };
  ...
  const _MakeAccordionItem = (obj) => {
    ...
    } else if (obj.label === "start_bid_amount") {
      item = (
        <AccordionItem key={obj.label} uuid={obj.label}>
          <AccordionItemHeading>
          ...
          <AccordionItemPanel>
            ...
            <div className="text-end">
              <button
                onClick={_handleSubmitPriceRange}
                className="bg-primary text-white text-sm font-bold hover:bg-opacity-90 py-1 px-4 rounded-md duration-300 mt-4"
              >
                Apply
              </button>
            </div>
